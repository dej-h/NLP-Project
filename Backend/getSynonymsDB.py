        ...
        if word in db_words:  # If the word exists in the database
            ...
            if synonyms:  # If synonyms are found
                for synonym_tuple in synonyms:
                    ...
                    syn = cursor.fetchone()
                    if syn is not None:
                        synonym_simplicity_score = syn[0]
                    else:
                        synonym_simplicity_score = "N/A"

                    cursor.execute("SELECT relatedness_score FROM synonyms WHERE word = ? AND synonym = ?", (word, synonym))
                    rel = cursor.fetchone()
                    if rel is not None:
                        relatedness_score = rel[0]
                    else:
                        relatedness_score = "N/A"
...
if __name__ == "__main__":
    input_text = sys.stdin.read()  # Read the text from stdin
    main(input_text)